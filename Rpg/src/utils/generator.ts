...
import {Armor} from "../class/item/armor/Armor";
...
const getRandomNameAndRace = (): { pseudo: string, race: Race } => {
...
const getRandomNameandType = (): {itemName: string, weaponType: WeaponType } => {
...
const RandomStats = (): number => Math.floor(Math.random() * 10) + 1;


const RandomEnemy = (): boolean => Math.random() < 0.5;

const RandomGrade = (): string => Math.random() < 0.5 ? "Common" : "Rare";

const RandomBasedDamage = (): number => Math.floor(Math.random() * 10) + 1;

const generateCharacter = (isPlayer: boolean = false): Creatures | Player => {
    ...
    const weapons: Weapons[] = [generateWeapon()];
    const armors: Armor[] = [];
...
const generatePnj = (): Creatures => {
    return generateCharacter();
};

const generatePlayer = (): Player => {
    return generateCharacter(true) as Player;
};
